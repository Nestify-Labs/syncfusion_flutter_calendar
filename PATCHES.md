...
- Base version: `32.1.23`
- Source: pub.dev (`https://pub.dev/api/archives/syncfusion_flutter_calendar-32.1.23.tar.gz`)
- Hosted package SHA-256: `af27535ba4d3c6e64a318172c8a76363c174863b4e2583b21555c47be92c315d`
- Scope excluded from `main`: `example/`, `screenshots/`, `syncfusion_flutter_calendar.iml`
  (These remain present on `upstream/32.1.x` for reference but are stripped on the consumable `main` branch so Nestify does not register them as Flutter app assets.)

## Patch Rules

- Do **not** modify `.pub-cache`. All patch work happens on `main`.
- Keep this fork at `32.1.23` unless a Syncfusion upgrade is being explicitly rebased.
