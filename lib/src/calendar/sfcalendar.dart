...
class SfCalendar extends StatefulWidget {
  ...
  /// [SF-14 Nestify patch] Exposes a command-style Schedule(list) scroll API to
  /// the host. When null, behavior remains byte-identical to upstream.
  final SfCalendarScheduleScrollApiReadyCallback? onScheduleScrollApiReady;
...
class _SfCalendarState extends State<SfCalendar>
    with
        ...
        SfCalendarTimelineQueryApi,
        SfCalendarScheduleScrollApi {
  ...
  /// [SF-14 Nestify patch] Absolute scroll-content Y for the Schedule(list)
  /// current-time indicator produced by the last rendered today row.
  double? _scheduleCurrentTimeIndicatorScrollOffset;
  ...
  void initState() {
    ...
    _dispatchScheduleScrollApiToHostInternal(this);
    super.initState();
  }
  ...
  void didUpdateWidget(SfCalendar oldWidget) {
    ...
    if (oldWidget.onScheduleScrollApiReady != widget.onScheduleScrollApiReady) {
      _dispatchScheduleScrollApiToHostInternal(
        null,
        callback: oldWidget.onScheduleScrollApiReady,
      );
      _dispatchScheduleScrollApiToHostInternal(this);
    }
  ...
  @override
  void dispose() {
    _dispatchScheduleScrollApiToHostInternal(null);
  ...
  // ─── [SF-14] Nestify patch: Schedule current-time scroll API ───────────

  void _dispatchScheduleScrollApiToHostInternal(
    SfCalendarScheduleScrollApi? api, {
    SfCalendarScheduleScrollApiReadyCallback? callback,
  }) {
    final SfCalendarScheduleScrollApiReadyCallback? cb =
        callback ?? widget.onScheduleScrollApiReady;
    if (cb == null) {
      return;
    }
    final SchedulerPhase phase = SchedulerBinding.instance.schedulerPhase;
  ...
  bool scrollScheduleCurrentTimeToFraction(double fraction) {
    ...
    final ScrollController? controller = _agendaScrollController;
    final double? indicatorOffset = _scheduleCurrentTimeIndicatorScrollOffset;
    if (controller == null ||
  ...
  Widget? _getItem(BuildContext context, int index, bool isRTL) {
    ...
    // [SF2227-diag] E: log the display week + neighbours to confirm SF-16 re-key
    // re-invokes _getItem with fresh subjects (residual gone) on each epoch bump,
    // and that no State rebuild happened (D/addAgenda hasClients stays true).
    if (index >= -1 && index <= 1) {
      debugPrint(
        '[SF2227-diag] E/getItem epoch=$_scheduleContentEpoch index=$index '
        'week=$startDate apptCount=${appointmentCollection.length} '
        'subjects=${appointmentCollection.map((CalendarAppointment a) => a.subject).take(8).toList()}',
      );
    }
    ...
    for (int i = 0; i < dateAppointmentKeys.length; i++) {
      ...
      if (widget.showCurrentTimeIndicator && isSameDate(currentDate, now)) {
        final double? indicatorY = scheduleCurrentTimeIndicatorYForAppointments(
          currentAppointments,
          now,
          agendaItemHeight: appointmentViewHeight,
          agendaAllDayItemHeight: allDayAppointmentHeight,
          isLargerScheduleUI: !_useMobilePlatformUI,
          allDayFirst: widget.agendaSortAllDayAppointmentsFirst,
        );
        if (indicatorY != null) {
          _scheduleCurrentTimeIndicatorScrollOffset =
              viewStartPosition + appointmentViewTopPadding + indicatorY;
        }
      }
  ...
  Widget _getDisplayDateView(
    ...
    if (showNowLine) {
      // [SF-14] Matches SF-10's bottom-aligned no-events current-time line.
      _scheduleCurrentTimeIndicatorScrollOffset =
          highlightViewStartPosition + displayDateHighlightHeight;
    }
    return MouseRegion(
      onEnter: (PointerEnterEvent event) {
        _pointerEnterEvent(
  ...
  Widget addAgenda(double height, bool isRTL) {
    ...
    debugPrint(
      '[SF2227-diag] D/addAgenda initialScrollOffset='
      '${_agendaScrollController!.initialScrollOffset} '
      'hasClients=${_agendaScrollController!.hasClients} '
      'nextDatesEmpty=${_nextDates.isEmpty} '
      'displayDateBlockWillRun=${_nextDates.isNotEmpty && _agendaScrollController!.initialScrollOffset == 0 && !_agendaScrollController!.hasClients}',
    );
...
class _CalendarHeaderViewState extends State<_CalendarHeaderView> {
  ...
  void _updateWeekNumberChangedForTimelineMonth() {
    ...
    // To avoid the unnecessary build while the timeline month view building
    SchedulerBinding.instance.addPostFrameCallback((Duration timeStamp) {
      setState(() {});
    });
  }
...
// ─────────────────────────────────────────────────────────────────────────
// [SF-14] Nestify patch: Schedule(list) current-time scroll API
//
// Exposes a narrow command handle so host apps can align the existing SF-10
// Schedule current-time indicator without reflecting Syncfusion internals.
// ─────────────────────────────────────────────────────────────────────────

/// Callback that receives the current Schedule scroll API handle.
///
/// The callback receives `null` when the previous handle is no longer valid.
typedef SfCalendarScheduleScrollApiReadyCallback =
    void Function(SfCalendarScheduleScrollApi? api);

/// Public mixin on [State] exposing Schedule(list) command-style scrolling.
mixin SfCalendarScheduleScrollApi on State<SfCalendar> {
  /// Scrolls the Schedule(list) current-time indicator to [fraction] of the
